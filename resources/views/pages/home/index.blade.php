...
{{-- Preload ONLY first hero slide for fastest LCP --}}
@push('preload')
@if($heroSlides->count() > 0)
@php
    $firstSlide = $heroSlides->first();
    $heroUrl = $firstSlide->image_url;
...
@section('content')
    ...
    @if($heroSlides->count() > 0)
    <section class="relative h-[380px] md:h-[480px] lg:h-[540px] overflow-hidden bg-primary-900 md:bg-transparent" x-data="{
        currentSlide: 0,
        totalSlides: {{ $heroSlides->count() }},
        autoplayInterval: null,
        init() {
            // Delay autoplay start to avoid interfering with LCP measurement on slow connections
        ...
        <div x-show="currentSlide === {{ $index }}"
            ...
            {{-- Content --}}
            <div class="container relative z-10 h-full flex items-center py-8 md:py-0">
                <div class="max-w-2xl text-{{ $slide->text_color }}">
                    @if($slide->badge_text)
                    <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm text-{{ $slide->text_color }} px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium mb-3 md:mb-5 border border-white/20">
                    ...
                    <h2 class="text-2xl md:text-4xl lg:text-5xl font-bold font-heading mb-3 md:mb-4 leading-tight">
                        {{ $slide->title }}
                    </h2>
                    
                    @if($slide->subtitle)
                    <p class="text-base md:text-lg lg:text-xl mb-4 md:mb-6 text-{{ $slide->text_color }}/90 leading-relaxed line-clamp-2 md:line-clamp-3">
                        {{ $slide->subtitle }}
                    </p>
                    @endif
        ...
        {{-- Dots Indicator --}}
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3" role="tablist" aria-label="{{ __('Slide navigation') }}">
            @foreach($heroSlides as $index => $slide)
            <button @click="goToSlide({{ $index }})" 
                    aria-label="{{ __('Go to slide') }} {{ $index + 1 }}"
                    role="tab"
                    :aria-selected="currentSlide === {{ $index }}"
                    class="transition-all rounded-full p-2 min-w-[44px] min-h-[44px] flex items-center justify-center"
                ...
                @foreach($testimonials as $testimonial)
                    <div class="group relative bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-2xl transition-all duration-300 hover:-translate-y-2" x-data x-intersect.once="$el.classList.add('animate-fade-in-up')">
                        ...
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            @endif
                            <div>
                                <h3 class="font-bold text-gray-900 text-base">{{ $testimonial->author_name }}</h3>
        ...
        <div class="container relative z-10">
            <div class="max-w-4xl mx-auto text-center">
                ...
                <div class="flex flex-wrap justify-center gap-4">
                    ...
                    <a href="https://wa.me/{{ $whatsappNumber }}?text={{ urlencode(__('Hello, I would like to ask about your products.')) }}" target="_blank" class="inline-flex items-center gap-2 bg-green-700 hover:bg-green-800 text-white px-8 py-4 rounded-xl font-bold transition-all hover:shadow-2xl hover:scale-105 text-lg border-2 border-green-800 hover:border-green-900">
